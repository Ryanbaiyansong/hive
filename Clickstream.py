...
'''
The aim of this script is to create tables for the ClickStream data
present in s3 bucket. This script creates 3 external tables, logs, 
users and products.
'''
class ClickStream:
    def main(self):
        ...
        hiveContext.sql("""CREATE EXTERNAL TABLE IF NOT EXISTS users 
(swid string, birth_dt string, gender_cd string) 
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED AS TEXTFILE 
LOCATION 's3n://AKIASHLL3WWNMYCAH6ZC:n2axVdSVz0G+6QyTyBXFdCPgTW87OXoLV93ItGTk@gyati/clickStream/users'""")

        hiveContext.sql("""CREATE EXTERNAL TABLE IF NOT EXISTS  products
(url string, category string) 
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED AS TEXTFILE 
LOCATION 's3n://AKIASHLL3WWNMYCAH6ZC:n2axVdSVz0G+6QyTyBXFdCPgTW87OXoLV93ItGTk@gyati/clickStream/products'""")

        hiveContext.sql("""CREATE EXTERNAL TABLE IF NOT EXISTS  omniturelogs
(
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED AS TEXTFILE 
LOCATION 's3n://AKIASHLL3WWNMYCAH6ZC:n2axVdSVz0G+6QyTyBXFdCPgTW87OXoLV93ItGTk@gyati/clickStream/omniture'""")

        hiveContext.sql("""CREATE VIEW IF NOT EXISTS omniture AS 
SELECT
col_2 ts,
col_8 ip,
col_13 url,
...
from omniturelogs""")
        ...
        hiveContext.sql("""CREATE TABLE IF NOT EXISTS loganalytics AS
SELECT
        o.swid,
        o.ts,
        o.url,
...
FROM 
        ...
        hiveContext.sql("""CREATE TABLE IF NOT EXISTS  user_time_spent AS 
SELECT 
swid, 
url,
cast(min(ts) as timestamp) start_time ,
cast(max(ts) as timestamp) end_time,
(unix_timestamp(max(ts))-unix_timestamp(min(ts)))/60/60 time_spent,
id FROM
(SELECT *,(num-row_number() over (partition by swid,url,to_date(ts) order by num)) as id 
FROM (select *, row_number() over ( order by swid,ts) num from loganalytics ) a) 
group by swid,url,id""")
